...
import NavigationDropdown from '@mapbox/dr-ui/navigation-dropdown';
import debounce from 'debounce';
import Tag from '@mapbox/dr-ui/tag';
    ...
    onScrollLive() {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            if (sections[i].getBoundingClientRect().bottom > 0) {
                ...
                const subheadings = sections[i].querySelectorAll(
                    `div.section-h3`
                );
                ...
                for (let s = 0; s < subheadings.length; s++) {
                    if (subheadings[s].getBoundingClientRect().bottom > 0) {
                        this.setState({
                            activeh3: subheadings[s].getElementsByTagName(
                                'h3'
                            )[0]
                                ? subheadings[s].getElementsByTagName('h3')[0]
                                      .id
                                : ''
                        });
                        return;
            ...
            props.contents.secondLevelItems.map(item => {
                ...
                return (
                    <li key={item.path} className="mb6">
                        ...
                        <ul
                            className={
                                openSubItems ? 'pl12 color-darken75' : 'none'
                            }
                        >
                            {subItems}
                        </ul>
        ...
        const firstLevelContent = props.contents.firstLevelItems.map(
            (page, index) => {
                ...
                const textClasses = classnames(
                    'pl12 py12 txt-bold txt-m flex-child',
                    {
                ...
                if (!isActive) {
                    icon = (
                        <div className="flex-child flex-child--no-shrink">
                            <Icon
                                name="chevron-down"
                                className="icon color-gray h24 w24"
                            />
                        </div>
                    );
                }
